...
for file_name in tqdm(file_names):
    ...
    signal, rate = torchaudio.load(
        os.path.join("/mnt/QNAP/comdav/DATA/DATA/LJSpeech/wavs/", file_name)
    )
    ...
    spectrogram_original = torch.stft(
        signal, n_fft=1024, hop_length=256, win_length=1024, return_complex=True
    )
    spectrogram_vocoded = torch.stft(
        waveform, n_fft=1024, hop_length=256, win_length=1024, return_complex=True
    )
    ...
    for start in range(0, f_max, band_width):
        ...
        combined_waveform = torch.istft(
            spectrogram_combined, n_fft=1024, hop_length=256, win_length=1024
        )
        torchaudio.save(
            f"LJSpeech_vocoded/{file_name}_vocoded_{start}-{end}.wav",
            combined_waveform.unsqueeze(0),
            22050,
        )
