...
def get_feats(metadata):
    ...
    selected_labels = (
        ...
        + labels[35000:39000]
        + labels[40000:44000]
    )  # + labels[5000:9000] + labels[5000:9000]
    print(selected_audios[4000:4010])
    for audio, label in tqdm(zip(selected_audios, selected_labels)):
        # print(audio)
        audio_loaded, _ = audio_processor.load_audio(
            os.path.join("LJSpeech_vocoded/", audio)
        )
...
def train_logReg(metadata):
    ...
    joblib.dump(model, "logReg_ckpts/logReg_vocoded_anyband.joblib")
