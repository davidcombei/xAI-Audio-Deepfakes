...
class LMACLoss(nn.Module):
    ...
    def loss_function(
        self, magnitude, y_coeff_rel, y_coeff_irrel, phase, class_pred
    ):  # , weights=None):
        ...
        # manual_mask = torch.tensor([[0., 0., 1., 0. ,0., 0., 0., 0.]], device=device)
        for i in range(8):
        ...
        # print("p(x * m): ",torch.sigmoid(rel_logits))
        # print("p(x * (1-m)): ",torch.sigmoid(irr_logits))
