...
model.load_state_dict(
    torch.load(
        "mask_predictor_bands_MLP/5-6k/band_5-6k_epoch_20_loss_56.1914.pth",
        map_location=device,
    )
)
...
def extract_wavs(metadata):
    ...
    ## metadata = 5-9k - 0-1 kHz; 10-14k 1-2 kHz, 15-19k 2-3kHz, 20-24k 3-4kHz, 25-29k 4-5kHz, 30-34kHz 5-6k ... (rest are for testing)
...
BATCH_SIZE = 16
dataset = AudioDataset(audio_processor=audio_processor, device=device)
...
eval_mask(model=model, data_loader=data_loader, correct_band=5)
